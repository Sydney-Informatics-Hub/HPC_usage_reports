...
# with the same prefix on Artemis. If no prefix is specified, a warning 
# wil be given, and the queue time will be reported for all jobs with 
# uasge logs found within the present directory. Please note that 
# queue time can only be reported up to 14 days post-completion, as
# PBS does not preserve historical records beyond this point.
...
#
# Usage:
# Run the script from within the directory that contains the usage log 
# files for the jobs of interest. Include the prefix of the usage logs 
# as an argument on the command line, eg:
# perl <path/to/script/artemis_queuetime_report.pl <prefix>
# Option 1 - script is run within 14 days of all jobs completing:
# No additional arguments are required
# Option 2 - 'qstat -xf $PBS_JOBID' has been included in the job script:
# Run the script with the additional flag 'q', eg:
# perl <path/to/script/artemis_queuetime_report.pl <prefix> -q
# This will cause the script to search for the required information 
# within the job output logs instead of using qstat.
# 
#Output:
...
# perl <path/to/script/artemis_queuetime_report.pl <prefix> > queue_times.txt
#
# Author: Cali Willet
# cali.willet@sydney.edu.au
#
# Date last modified: 02/06/2019
#
