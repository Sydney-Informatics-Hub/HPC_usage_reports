...
#------------------------------------------------------------------
# raijin_usage_report/1.0 
# Platform: NCI Raijin HPC
#
# Description: 
# This script gathers the job requests and usage metrics from Raijin log 
...
# command line, eg:
# perl <path/to/script/raijin_usage_report.pl <prefix>
#
# Output:
...
# perl <path/to/script/raijin_usage_report.pl <prefix> > resources_summary.txt  
...
foreach my $file (@logs) { 
	...
	my $e = 0; 
	if ($cpus_used!~m/unknown/) {  #not sure if this 'unknown' report ever happens on Raijin like it does on Artemis...
	...
	#my $SUs = sprintf("%.2f", (($walltime_mins/60)*$cpus_req)); #Don't need to do the calculation on Raijin as it's in the log
